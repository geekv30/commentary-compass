...
description: LaTeX authoring contract for Examlly question and solution diagrams — package stack, boilerplate, style conventions, worked examples, and failure modes. Invoke BEFORE writing any `.tex` source for a diagram referenced by a `{{diagram:key}}` token in extract-questions-guide or author-solution-guide.
allowed-tools:
  - Read
  - Bash
---
...
Shared LaTeX contract for the `diagrams[*].source` field emitted by extract-questions-guide and author-solution-guide. Invoke this skill whenever you are about to write `.tex` for a new `diagrams` entry.
