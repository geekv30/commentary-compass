...
2. Delete `0000_baseline.sql` and its snapshot
3. Run `drizzle-kit generate` — it produces a fresh baseline
4. If the change requires new functions/triggers/grants, edit `0001_custom-sql.sql` in place
5. If the change requires new reference data, edit `0002_seed-reference-data.sql` in place
6. Run `make db-reset` to verify
